...
`.trim(), async() => {
  ...
  test(
    `[Util.touch-003]
  Given
    - an existing filename with content
  When
    - we run touch() on the existing filename
  Then
    - we return true
...
`.trim(), async() => {
  ...
  const folder = tmpobj.name;
  const filename = path.join(folder, "myfile.txt");
  const expected_content = "some content";
  Util.touch(filename);
  fs.writeFileSync(filename, expected_content, "utf8");
  ...
  expect(result).toEqual(true);
  expect(fs.existsSync(filename)).toEqual(true);
  expect(content).toEqual(expected_content);
});
